...
runoff_summary
runoff_day[, year := year(date)] # create a column with just the year
runoff_day[, month := month(date)] #create a column with the month

runoff_month <- runoff_day[, .(value = mean(value)), by = .(month, year, sname)]
runoff_month[, date := as.Date(paste0(year, '-', month, '-1'))] # create a column with the date format
